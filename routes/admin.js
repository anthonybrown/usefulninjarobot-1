...
exports.new = function(req, res){
  if(req.session.admin == 'true'){
    post.find({}).sort('-_id').execFind(function(err, posts){
      if(posts){
        res.render('admin', { title: title, subTitle:sub, posts:posts, admin:req.session.admin});
      }else{
        res.render('admin', { title: title, subTitle:sub, posts:null, admin:req.session.admin })
      }
    });
  }else{
    res.redirect('/')
...
exports.delete = function(req, res){
    if (req.session.admin == 'true'){
      post.find({}).sort('-_id').execFind(function(err, posts){
      if(posts){
        res.render('admin_delete', { title: title, subTitle:sub, posts:posts, admin:req.session.admin});
      }else{
        res.render('admin_delete', { title: title, subTitle:sub, posts:null, admin:req.session.admin })
      }
      });
    }else{
      res.redirect('/')
