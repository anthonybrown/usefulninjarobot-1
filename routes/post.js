	...
	post.find({'_id': id}, function(err, post){
      if(post){
      	commentss.find({'postid': id}, function(err, comment){
      		if(comment){
      			res.render('post_view', {title:title, subTitle:sub, post:post, comment:comment, admin:req.session.admin})
      		}else{
      			res.render('post_view', {title:title, subTitle:sub, post:post, comment:null, admin:req.session.admin})
      		}
      	});

      }else{
        res.render('post_view', {title:title, subTitle:sub, post:null, comment:null, admin:req.session.admin})
      }
    });
}
